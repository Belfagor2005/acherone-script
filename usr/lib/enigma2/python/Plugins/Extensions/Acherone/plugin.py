...
class OpenScript(Screen):
    ...
    def getScrip(self, url):
        ...
        try:
            ...
            self.session.open(
                MessageBox, _("Network error: %s") %
                str(e), MessageBox.TYPE_ERROR)
        except Exception as e:
            self.session.open(MessageBox, str(e), MessageBox.TYPE_ERROR)

    def sremove(self):
        self.session.openWithCallback(self.xremove, MessageBox, _(
            'Remove all scripts from folder?'), MessageBox.TYPE_YESNO)
    ...
    def xremove(self, answer=False):
        if answer:
            ...
            self.session.open(
                MessageBox,
                _('Scripts successfully removed!'),
                MessageBox.TYPE_INFO)
    ...
    def on_select(self):
        if len(self.mlist) > 0:
            self.session.openWithCallback(
                self.selectsc,
                MessageBox,
                _('Execute Script Selected?'),
                MessageBox.TYPE_YESNO)
        else:
            self.session.open(
                MessageBox,
                _('Please Download Script!'),
                MessageBox.TYPE_INFO)
...
def Plugins(**kwargs):
    return [
        PluginDescriptor(
            name="Acherone Script",
            description=_("Openscript by Lululla"),
